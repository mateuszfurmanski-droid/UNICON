...
# ===== UNICON: ARUCO IN MASTER (V1) =====
# Adds:
#   GET /api/aruco/test_v1
#   GET /api/aruco/state_v1
try:
    import cv2
    import numpy as np
    ...
    def _aruco_pick_frame():
        # Prefer RIGHT frame if present, then LEFT. (Do not open cameras here.)
        fr = None
        if 'frame_right' in globals() and globals().get('frame_right') is not None:
            fr = globals()['frame_right']
        elif 'frame_left' in globals() and globals().get('frame_left') is not None:
            fr = globals()['frame_left']
        return None if fr is None else fr.copy()

    @app.route("/api/aruco/test_v1")
    def api_aruco_test_v1():
        try:
            frame = _aruco_pick_frame()
            if frame is None:
            ...
            return jsonify({"ok": True, "markers": int(len(ids)), "ids": [int(x) for x in ids.flatten().tolist()]})
        except Exception as e:
            return jsonify({"ok": False, "err": f"aruco_exception:{type(e).__name__}:{e}"}), 500

    @app.route("/api/aruco/state_v1")
    def api_aruco_state_v1():
        try:
            has_r = bool('frame_right' in globals() and globals().get('frame_right') is not None)
            has_l = bool('frame_left' in globals() and globals().get('frame_left') is not None)
            return jsonify({"ok": True, "has_frame_right": has_r, "has_frame_left": has_l})
        except Exception as e:
            return jsonify({"ok": False, "err": f"state_exception:{type(e).__name__}:{e}"}), 500

except Exception:
    # If OpenCV aruco isn't available, keep master alive.
    pass
