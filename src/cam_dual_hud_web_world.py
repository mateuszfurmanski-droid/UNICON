  ...
  <div id="wrap">
    ...
    <div id="hud">
      ...
      <div style="margin-top:6px;">
        ...
        <a href="/_i/btn2/state">/_i/btn2/state</a>
      </div>
    </div>
  </div>
...
document.getElementById('tapLayer').addEventListener('click', (ev)=>{
  ...
  if(mode === 'A'){
    fetch(`/_i/btn2/A?x=${x}&y=${y}`).catch(()=>0);
    placeCross('crossA', parseFloat(x), parseFloat(y));
  } else {
    fetch(`/_i/btn2/B?x=${x}&y=${y}`).catch(()=>0);
...
async function poll(){
  try{
    const r = await fetch('/_i/btn2/state', {cache:'no-store'});
...
def api_btn(name):
    ...
    # Some UIs call /_i/btn2/state (GET) without btn_name in view args.
    try:
...
@app.route("/_i/btn2/state", methods=["GET"])
def api_btn2_state():
...
try:
    ...
    bp_world.__globals__["__UNICON_MAIN_GLOBALS__"] = globals()
    app.register_blueprint(bp_world)
except Exception as e:
    print("WORLD_ARUCO module load failed:", e)
...
# Safe: loads extra endpoints from /home/pi/UNICON/modules without touching core logic.
try:
    from modules.tool_luna_now import register_tool_luna_now
    register_tool_luna_now(app, globals())
except Exception as _e:
    print("MODULE_LOAD tool_luna_now FAILED:", _e)

try:
    from unicon_3d_overlay import bp_3d
    app.register_blueprint(bp_3d)
except Exception as e:
    print("3D module load failed:", e)

try:
    from unicon_3d_core import bp_3d
    app.register_blueprint(bp_3d)
except Exception as e:
    print("3D core load failed:", e)
...
# === UNICON WORLD: minimal test endpoint (no baseline edits) ===
try:
    from flask import jsonify

    @app.route("/api/world/aruco_test")
    def _unicon_world_aruco_test():
        return jsonify(ok=True, aruco=True)
except Exception as e:
    print("[WORLD] inject /api/world/aruco_test failed:", e)

# === UNICON WORLD: aruco detect on RIGHT cam (single frame) ===
try:
    import cv2
    import numpy as np
    from flask import jsonify, request

    @app.route("/api/world/aruco_detect")
    def _unicon_world_aruco_detect():
        # optional: ?dict=4X4_50
        dname = (request.args.get("dict") or "4X4_50").upper()
        dict_map = {
        ...
        dkey = dict_map.get(dname, cv2.aruco.DICT_4X4_50)
        ...
        ok, frame = cap.read()
        if not ok or frame is None:
            return jsonify(ok=False, err="no_frame"), 500
    ...
    print("[WORLD] inject /api/world/aruco_detect failed:", e)
