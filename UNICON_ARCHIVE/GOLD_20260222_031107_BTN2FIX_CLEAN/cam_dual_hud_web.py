  ...
  <div id="wrap">
    ...
    <div id="hud">
      ...
      <div style="margin-top:6px;">
        ...
        <a href="/api/btn2fix/state">/api/btn2fix/state</a>
      </div>
    </div>
  </div>
...
document.getElementById('btnR').onclick = ()=>{
  fetch('/api/btn2fix/RESET').catch(()=>0);
...
document.getElementById('tapLayer').addEventListener('click', (ev)=>{
  ...
  if(mode === 'A'){
    fetch(`/api/btn2fix/A?x=${x}&y=${y}`).catch(()=>0);
    placeCross('crossA', parseFloat(x), parseFloat(y));
  } else {
    fetch(`/api/btn2fix/B?x=${x}&y=${y}`).catch(()=>0);
...
async function poll(){
  try{
    const r = await fetch('/api/btn2fix/state', {cache:'no-store'});
...
def api_btn(name):
    ...
    # Some UIs call /api/btn2fix/state (GET) without btn_name in view args.
    try:
...
@app.route("/api/btn2/<name>", methods=["GET","POST"])
def api_btn2(name):
    name = (name or "").upper()
...
@app.route("/api/btn2fix/state", methods=["GET"])
def api_btn2_state():
